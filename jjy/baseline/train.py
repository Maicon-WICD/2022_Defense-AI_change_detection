...
if __name__ == '__main__':
    ...
    train_result_dir = os.path.join(prj_dir, 'results', 'train', config['save_dir_name'])
    ...
    # Set data directory
    train_dirs = os.path.join(config['data_path'])
    ...
    model = model(classes=config['n_classes'],
                ...
                encoder_output_stride=config['stride'],
                encoder_depth=config['depth']).to(device)
    ...
    for epoch_id in range(config['n_epochs']):
        ...
        # save weight
        recorder.save_weight(epoch=epoch_id,val_miou=row['val_miou'])
