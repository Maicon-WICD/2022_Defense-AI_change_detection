...
if __name__ == '__main__':
    ...
    train_config = load_yaml(os.path.join(prj_dir, config['exp'], 'train.yaml'))
    ...
    pred_result_dir = os.path.join(prj_dir, config['output_path'])
    pred_result_dir_mask = os.path.join(prj_dir, config['output_path'], 'mask')
    ...
    # Set data directory
    test_dirs = os.path.join(config['data_path'])
    ...
    model = model(
                ...
                encoder_output_stride=train_config['stride']).to(device)
    logger.info(f"Load model architecture: {train_config['architecture']}")

    #! Load weight
    check_point_path = os.path.join(prj_dir, config['exp'], 'model.pt')
